...
import { useAboutData, setItem } from "../../hooks/useFirestoreData";

const AboutEditor = () => {
  const { data: about, loading } = useAboutData();
  ...
  const handleSave = async () => {
    ...
    try {
      const { id, ...data } = form;
      await setItem("siteConfig", "about", data);
      setSaved(true);
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="text-3xl font-bold text-white tracking-wider">About Me</h1>
        <p className="text-[#aaa6c3] text-sm mt-1">Edit your about section paragraph</p>
      </motion.div>
      ...
      >
        <div>
          <label className="block text-[#aaa6c3] text-xs uppercase tracking-widest mb-2">Section Heading</label>
          <input
            type="text"
            value={form.heading || ""}
        ...
        <div>
          <label className="block text-[#aaa6c3] text-xs uppercase tracking-widest mb-2">About Text</label>
          <textarea
            value={form.text || ""}
          ...
          />
          <p className="text-[#aaa6c3] text-xs mt-1">{(form.text || "").length} characters</p>
        </div>
